...
public class SimpleGameController : MonoBehaviour {
  ...
  void Start() {
    fTime = Time.time;
    fPos = GameObject.Find("BigBall").transform.position;
  }

  void FixedUpdate() {
    if ((Time.time - fTime) > 20.0f) {
      GameObject.Find("BigBall").transform.position = fPos;
      fTime = Time.time;
    }
  }
    ...
    if (!Network.isClient && !Network.isServer && !isClientStart && !isLocal) {
      ...
      if (GUI.Button (new Rect (100, 320, 250, 100), "Start Local")) {
        ...
        if (terrainObject != null) {
          ...
          TerrainData td = Object.Instantiate(t.terrainData) as TerrainData;
          td.name = t.terrainData.name + "_copied";
          t.terrainData = td;
          t.Flush();
          TerrainCollider tc = Terrain.activeTerrain.gameObject.GetComponent<TerrainCollider>();
          tc.terrainData = td;
          Debug.Log("terrain copied!");
        }
        */
